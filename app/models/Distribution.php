...
class Distribution
{
    ...
    public static function simulerDistribution(PDO $db, string $logic = 'ancien'): array
    {
        ...
        if ($logic === 'petit') {
            return self::simulerPetit($db, $besoins);
        }
        return self::simulerAncien($db, $besoins);
    }

    /**
     * Logique "plus petit" : priorise les besoins avec la plus petite quantité restante
     */
    private static function simulerPetit(PDO $db, array $besoins): array
    {
        $resultats = [];
        // Trier les besoins par quantité croissante, puis par id
        usort($besoins, function ($a, $b) {
            return ($a['quantite'] <=> $b['quantite']) ?: ($a['id'] <=> $b['id']);
        });
        foreach ($besoins as $besoin) {
            $sqlRecu = "SELECT COALESCE(SUM(quantite),0) FROM bngrc_distribution WHERE besoin_id = :besoin_id AND est_simulation = FALSE";
            $stmtRecu = $db->prepare($sqlRecu);
            $stmtRecu->execute([':besoin_id' => $besoin['id']]);
            $reste = $besoin['quantite'] - (int)$stmtRecu->fetchColumn();
            if ($reste <= 0) continue;

            $sqlDons = "SELECT * FROM v_bngrc_dons_disponibles_par_type WHERE type_article_id = :type_article_id ORDER BY date_don ASC, id ASC";
